;(function(){//+
	...
	*/
	var getStatus = function(context, isDefault){
		...
		//init vars	
		min = context.params.$uiSlider.get(0).noUiSlider.options.range.min[0];
		max = context.params.$uiSlider.get(0).noUiSlider.options.range.max[0];	
		...
		if(isDefault){
			...
			prev = context.params.$uiSlider.get(0).noUiSlider.get()[0]; 
			next = context.params.$uiSlider.get(0).noUiSlider.get()[1];
		}
	...
	var setStatus = function(context, status, restoredFromStorage){
		...
		if(jQuery.isNumeric(status.data.prev) && jQuery.isNumeric(status.data.next)){
			...
			if(!isNaN(prev) && !isNaN(next)){	
				...
				if(context.params.$uiSlider.get(0).noUiSlider.get()[0] != prev){
					context.params.$uiSlider.get(0).noUiSlider.set([prev, null]);
				}
				
				if(context.params.$uiSlider.get(0).noUiSlider.get()[1] != next){
					context.params.$uiSlider.get(0).noUiSlider.set([null, next]);
				}
			}
		}
	...
	*/
	var initEvents = function(context){
		...
		//on value change
		context.params.$uiSlider.get(0).noUiSlider.on('update', function(){
	...
	*/
	var Init = function(context){
		...
		context.params.defaultPrev = context.params.$uiSlider.get(0).noUiSlider.get()[0]; 
		context.params.defaultNext = context.params.$uiSlider.get(0).noUiSlider.get()[1]; 
